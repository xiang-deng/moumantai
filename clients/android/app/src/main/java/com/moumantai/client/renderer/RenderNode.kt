...
import com.moumantai.client.renderer.renderers.BoxRenderer
import com.moumantai.client.renderer.renderers.ButtonRenderer
import com.moumantai.client.renderer.renderers.CardRenderer
import com.moumantai.client.renderer.renderers.CheckBoxRenderer
import com.moumantai.client.renderer.renderers.ChipRenderer
import com.moumantai.client.renderer.renderers.ColumnRenderer
import com.moumantai.client.renderer.renderers.DateTimeInputRenderer
import com.moumantai.client.renderer.renderers.DividerRenderer
import com.moumantai.client.renderer.renderers.FabRenderer
import com.moumantai.client.renderer.renderers.IconRenderer
import com.moumantai.client.renderer.renderers.ImageRenderer
import com.moumantai.client.renderer.renderers.ListItemRenderer
import com.moumantai.client.renderer.renderers.ListRenderer
import com.moumantai.client.renderer.renderers.ModalRenderer
import com.moumantai.client.renderer.renderers.ProgressBarRenderer
import com.moumantai.client.renderer.renderers.ProgressRingRenderer
import com.moumantai.client.renderer.renderers.RowRenderer
import com.moumantai.client.renderer.renderers.ScaffoldRenderer
import com.moumantai.client.renderer.renderers.SelectRenderer
import com.moumantai.client.renderer.renderers.SliderRenderer
import com.moumantai.client.renderer.renderers.SwitchRenderer
import com.moumantai.client.renderer.renderers.TabsRenderer
import com.moumantai.client.renderer.renderers.TextFieldRenderer
import com.moumantai.client.renderer.renderers.TextRenderer
import com.moumantai.client.renderer.renderers.TopBarRenderer
...
/**
 * Core recursive renderer for the Moumantai component tree.
 *
 * Looks up a [ComponentDef] by ID, dispatches to the appropriate Compose
 * renderer based on which variant field is non-null on the Wire message,
 * and recurses for container components. The `when` in [dispatchComponent]
 * covers all known variants; unknown ones are skipped for forward-compat.
 */

/**
 * Render a single Moumantai component by its ID.
 *
 * @param componentId The ID of the component to render.
 * @param components All components for this surface, keyed by ID.
 * @param data The surface's data model.
 * @param surfaceId The current surface ID (used for action dispatch).
 * @param itemScope Item scope data for list item templates (null for top-level components).
 * @param itemScopePath JSON Pointer path to the current item (e.g. "/items/0").
 * @param dispatch Callback to send typed [ActionDef]s to the transport layer.
 * @param parent Position of this child in the render tree — passed by the
 *               caller (a container renderer, or [RenderParent.ROOT] at the
 *               surface root). The catalog-driven layout resolver
 *               ([resolveModifierWithSize]) reads this to compute cross-axis
 *               sizing per shared/protocol/spec.md rule 10.
 */
@Composable
fun RenderNode(
    componentId: String,
...
/**
 * Pull the `Modifier` off whatever variant of [ComponentDef] is set. Returned
 * lazily so the caller can also use it for the visibility gate before
...
/**
 * Catalog component-kind name (PascalCase) for the variant set on this
 * [ComponentDef]. Mirrors the TS-side `variantCaseToKind`; used by
 * [Layout.containerChildGap] and [resolveModifierWithSize].
 */
...
/**
 * Dispatch to the per-variant renderer. Wire emits nullable fields (not a
 * sealed enum), so the compiler won't flag a missing variant — the
 * `RendererExhaustivenessTest` catches gaps instead.
 */
@Composable
internal fun dispatchComponent(
    ...
    when {
        ...
        def.text_field != null -> TextFieldRenderer(def.id, def.text_field, parent, data, itemScope)
        ...
        def.slider != null -> SliderRenderer(def.id, def.slider, parent, data, itemScope, dispatch)
        ...
        def.date_time_input != null -> DateTimeInputRenderer(def.id, def.date_time_input, parent, data, itemScope, dispatch)
