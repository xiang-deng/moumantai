...
// Mirrors clients/android/.../MaterialIcons.kt — kept in sync manually until the
// two clients share a Kotlin module. Resolves Material Symbols names emitted
// by the server to Compose ImageVectors. `fa:` names are not supported on
// Wear and fall back to HelpOutline.
fun lookupMaterialIcon(name: String?): ImageVector {
    if (name.isNullOrBlank()) return Icons.Rounded.HelpOutline
    val raw = name.trim()
    // `fa:` prefix is the FontAwesome escape hatch. Wear can't render FA yet,
    // so surface HelpOutline rather than crash or fall back to the wrong glyph.
