...
class RenderNodeTest {
    ...
    fun `coversEveryComponentVariant`() {
        ...
        val variants: List<ComponentDef> = listOf(
    ...
    // -----------------------------------------------------------------------
    // parseBoxAlignment — pin the 9-value vocabulary the BoxRenderer relies
    // on. If a future YAML edit renames an alignment value (or this parser
    // drifts away from the catalog), this test fails before runtime breakage.
    // -----------------------------------------------------------------------

    @Test
    fun `parseBoxAlignment maps the 9 catalog alignment strings`() {
        assertEquals(Alignment.TopStart, parseBoxAlignment("topStart"))
        assertEquals(Alignment.TopCenter, parseBoxAlignment("topCenter"))
        assertEquals(Alignment.TopEnd, parseBoxAlignment("topEnd"))
        assertEquals(Alignment.CenterStart, parseBoxAlignment("centerStart"))
        assertEquals(Alignment.Center, parseBoxAlignment("center"))
        assertEquals(Alignment.CenterEnd, parseBoxAlignment("centerEnd"))
        assertEquals(Alignment.BottomStart, parseBoxAlignment("bottomStart"))
        assertEquals(Alignment.BottomCenter, parseBoxAlignment("bottomCenter"))
        assertEquals(Alignment.BottomEnd, parseBoxAlignment("bottomEnd"))
    }

    @Test
    fun `parseBoxAlignment unknown string falls back to TopStart (catalog default)`() {
        assertEquals(Alignment.TopStart, parseBoxAlignment("not_a_real_alignment"))
        assertEquals(Alignment.TopStart, parseBoxAlignment(""))
    }
}
